...
const qEach = (query, ctxOrFn, fn) => {
	(fn ? ctxOrFn : document).querySelectorAll(query).forEach(fn || ctxOrFn);
};
...
function * _genPlaceholders() {
	for (const item of RANDOM_PLACEHOLDERS) {
		yield item;
	}
...
function insertRedirect(pathname, destination) {
	...
	pathInput.parentNode.innerHTML = window.matchMedia('(min-width: 640px)').matches ? `https://${form.domain}/${pathname}` : `/${pathname}`;
	const destInput = q('input[name=destination]', nextLine);
	destInput.parentNode.innerHTML = destination;
	q('[data-copy]', nextLine).dataset.copy = `https://${form.domain}/${pathname}`;
...
q('input[name=pathname]').addEventListener('keypress', e => {
	if (e.key === 'Enter') {
		q('input[name=destination]').focus();
	}
});
